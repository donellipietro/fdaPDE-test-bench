...
using calibration_policy_t = int;
calibration_policy_t get_fpca_calibration(const std::string& calibration_name) {
    if (calibration_name == "gcv") return OptimizeGCV;
    else if (calibration_name == "kcv") return OptimizeMSRE;
    else return 0x1;
}
...
int main() {
    ...
    std::string calibration_name = json_file["RunParams"].value("calibration_policy","gcv");
    int mean_computation_option;
    if (json_file.at("RunParams").at("compute_mean").get<bool>()) mean_computation_option = 0;
    else mean_computation_option = DoNotComputeMean;

    fpca.fit(
        /* n_comp = */ json_file["RunParams"].value("n_pc",6),
        lambda_grid,
        /* options = */ ComputeRandSVD | get_fpca_calibration(calibration_name) | mean_computation_option,
