  ...
  for(solver_name in test_options$model_names){
    ...
    if (file.exists(file_model) && !FORCE_FIT) {
      ...
      for(j in 1:length(test_options$regularization$lambda_grid)){
        cpp_params_json$RunParams$lambda_grid <- I(test_options$regularization$lambda_grid[j])
      ...
      cpp_params_json$RunParams$lambda_grid <- test_options$regularization$lambda_grid
      ## save results
