...
## room for solutions
names_columns <- c("Group", "n_nodes", "n_locs", "n_stat_units", "NSR", names_models)
...
for (file_test in file_test_vect) {
  ...
  for (i in 1:n_reps) {
    ...
    ## rmse
    for (name in c("centering_locs", "centering")) {
...
## names
name_aggregation_option_vect <- c("n_nodes", "n_stat_units", "n_locs", "NSR")
name_group_vect <- c("K", "N", "S","NSR")
...
for (i in 1:length(name_aggregation_option_vect)) {
  ...
  mg <- expand.grid(options_grid_selected[[1]], options_grid_selected[[2]], options_grid_selected[[3]])
  combinations_options <- data.frame(as.vector(mg[[1]]), as.vector(mg[[2]]), options_grid_selected[[3]])
  ...
  for (j in 1:nrow(combinations_options)) {
    ...
    boxplot_list[[j]] <- plot.grouped_boxplots(
      ...
      limits = limits,
      LEGEND = T
    ) + standard_plot_settings() + theme(legend.position = "top")
