...
#' - put the eigenfunctions in test options, using something as function factory
#' - move the main.R to the root directory, making it general for any test to be performed, same for the post-processing script
...
#   translated_laplacian_eigenfunction(locs,i,x_t=0.0,y_t=0.0, ab_grid=ab_grid)
# }
# L <- anis_diff_op_2D(0.3,1.5)
# f <- function(points) { return(0*points[,1])} #forcing term set to 0
# fpc_indexes <- c(4,9,10)
# eig_diff_op_2D <- eigenfunctions.Lop(generated_domain$femr_mesh$nodes(),fpc_indexes,generated_domain$femr_mesh,L,f)
# 
# 
# loadings_true_generator <- function(locs,i){
#   Vh <- FunctionSpace(generated_domain$femr_mesh, fe_order=1)
#   return(as.matrix(Vh$basis()$eval(as.matrix(locs)) %*% eig_diff_op_2D[,i]))
# }
...
ab_grid <- matrix(c(1,1,
                  ...
                  ncol = 2, byrow = T)
loadings_true_generator <- function(locs,i) { 
  translated_laplacian_eigenfunction(locs,i,x_t=0.2,y_t=0,ab_grid = ab_grid)
}
...
if (RUN$tests) {
  for (batch_idx in 1:test_options$dimensions$n_reps) {
    ## message
    cat(paste("\nBatch ", batch_idx, ":\n", sep = ""))
...
colors <- brewer.pal(length(test_options$model_labels), "Set3")

### quantitative analysis ----
cat.subsection_title("Quantitative analysis")

if (RUN$quantitative_analysis) {
  pdf(file = paste(path_images, test_options$name_test, "_quantitative.pdf", sep = ""))
  source(paste("tests/", test_suite, "/templates/plot_quantitative_results.R", sep = ""))
  plot_quantitative_analysis(quantitative_analysis)
  dev.off()
}
