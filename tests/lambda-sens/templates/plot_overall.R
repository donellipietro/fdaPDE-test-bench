plot_overall <- function(name_aggregation_option_vect, name_group_vect, data_plot, title_vect, colors, 
                         limits=NULL, 
                         show_boxplot=T, show_lineplot=T){
  ## options
  options_grid <- list()
  ...
  for (i in 1:length(name_aggregation_option_vect)) {
    plots[[name_aggregation_option_vect[i]]] <- list()
    boxplot_list <- list()  #grouped boxplots
    violin_list <- list()  #grouped violinplots
    ...
    for (j in 1:nrow(combinations_options)) {
      ...
      violin_list[[j]] <- plot.grouped_violins(
        data_plot_trimmed[, c("Group", valid_models)],
        values_name = NULL,
        group_name = group_name,
        subgroup_name = "Approaches",
    ...
    plots[[name_aggregation_option_vect[i]]]$boxplot_list <- boxplot_list
    boxplot_grid <- arrangeGrob(grobs = boxplot_list, ncol = ncols)
    boxplot_grid <- labled_plots_grid(boxplot_grid, title, labels_cols, labels_rows, 7, 7)
    if(show_boxplot) grid.arrange(boxplot_grid)
    
    plots[[name_aggregation_option_vect[i]]]$violin_list <- violin_list
    violin_grid <- arrangeGrob(grobs = violin_list, ncol = ncols)
    violin_grid <- labled_plots_grid(violin_grid, title, labels_cols, labels_rows, 7, 7)
    if(show_boxplot) grid.arrange(violin_grid)

    plots[[name_aggregation_option_vect[i]]]$lineplot_list <- plot_list
    lineplot_grid <- arrangeGrob(grobs = plot_list, ncol = ncols)
    lineplot_grid <- labled_plots_grid(lineplot_grid, title, labels_cols, labels_rows, 9, 7)
    if(show_lineplot) grid.arrange(lineplot_grid)
  }
  return(plots)
}
