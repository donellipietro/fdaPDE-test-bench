...
if (length(args) == 0) {
  ...
  args[2] <- sort(list.files(path_queue),decreasing = T)[30]
}
...
fpc_specific_noise <- parsed_json$noise$fpc_specific_noise
...
cpp_scripts_path <- "/Users/marcogalliani/Projects/fdaPDE-test-bench/tests/lambda-sensibility/cpp-scripts/"
params_json <- fromJSON(paste(cpp_scripts_path, "params.json", sep = ""))

params_json$RunParams$lambda_grid <- I(lambda)
