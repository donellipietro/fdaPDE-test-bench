...
## room for solutions
names_columns <- c("Group", "NSR", "lambda", names_models)
...
for (file_test in file_test_vect) {
  ...
  lambda <- parsed_json$regularization$lambda
...
name_group_vect <- c("NSR", "lambda")
labels_aggregation_option_vect <- c(
...
title_vect <- paste(
  ...
  labels_aggregation_option_vect
)
...
source(paste("tests/", test_suite, "/templates/plot_fpcs_error.R", sep = ""))
