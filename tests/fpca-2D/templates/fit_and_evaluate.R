fit_and_evaluate_models <- function(path_list, 
  ...
  ## cpp params json
  cpp_params_json <- fromJSON(paste(cal_fpca_path, "params.json", sep = ""))
  ...
  for(solver_name in test_options$model_names){
    ...
    if (file.exists(file_model) && !FORCE_FIT) {
      ...
      params_json$RunParams$n_pc <-test_options$regularization$n_comp
      params_json$RunParams$lambda_grid <- test_options$regularization$lambda_grid
      ...
      model_fPCA$results$X_mean <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/center.csv",sep="")))
      model_fPCA$results$X_mean_locs <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/center_locs.csv",sep="")))
      ## 2) loadings
      model_fPCA$results$loadings <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/loadings.csv",sep="")))
      model_fPCA$results$loadings_locs <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/loadings_locs.csv",sep="")))
      #model_fPCA$results$loadings_norms <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/loadings_norms.csv",sep="")))
      ## 3) scores
      model_fPCA$results$scores <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/scores.csv",sep="")))
      
      ## 4) reconstruction
      model_fPCA$results$X_hat <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/reconstruction.csv",sep="")))
      model_fPCA$results$X_hat_locs <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/reconstruction_at_locs.csv",sep="")))
      ## 5) lambda
      model_fPCA$results$lambda <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/lambda.csv",sep="")))
...
# cal_fpca_path <- paste(cpp_scripts_path,"calibrated-fpca/",sep="")
# 
# ## load results_evaluation if available
...
#     ## 3) scores
#     model_fPCA$results$scores <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/scores.csv",sep="")))
#     model_fPCA$results$proj_scores <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/proj_scores.csv",sep="")))
#     ## 4) reconstruction
#     model_fPCA$results$X_hat <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/reconstruction.csv",sep="")))
#     model_fPCA$results$X_hat_locs <- as.matrix(read.csv(paste(cal_fpca_path,"/test-results/reconstruction_at_locs.csv",sep="")))
#     ## 5) lambda
