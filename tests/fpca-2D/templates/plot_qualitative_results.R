...
plot_qualitative_results <- function(qualitative_results, quantitative_results, domain){
  ...
  domain_boundary <- domain$domain_boundary
  ...
  for (m in 1:length(model_names)) {
    ...
    lable_model <- model_labels[m]
    plot_list <- list()
    plot_list_locs <- list()
    plot_list_HR_clean <- list()
    plot_list_HR <- list()
    indexes <- tapply(
      quantitative_results$rmses$loadings_locs[[name_model]],
      quantitative_results$rmses$loadings_locs$Group,
      function(x) {
        sapply(quantile(x, c(0, 0.5, 1),na.rm=T), function(q) which.min(abs(x - q)))
      }
    )
    limits_HR <- apply(
    ...
    for (i in 1:n_comp) {
      ...
      # at nodes (POINTS)
      plot_list[[4 * (i - 1) + 1]] <- plot.field_points(qualitative_results$nodes, loadings_true[, i], boundary = domain_boundary, size = 1.5, LEGEND = FALSE) +
        standard_plot_settings_fields()
      # at locs (POINTS)
      plot_list_locs[[4 * (i - 1) + 1]] <- plot.field_points(qualitative_results$locations, loadings_true_locs[, i], boundary = domain_boundary, size = 1.5, LEGEND = FALSE) +
        standard_plot_settings_fields()
      # HR grid (TILES)
      plot_list_HR_clean[[4 * (i - 1) + 1]] <- plot.field_tile(qualitative_results$grid, loadings_true_HR[, i], boundary = domain_boundary, LEGEND = FALSE) +
        standard_plot_settings_fields()
      # HR grid + ISOLINES (TILES)
      plot_list_HR[[4 * (i - 1) + 1]] <- plot.field_tile(qualitative_results$grid, loadings_true_HR[, i], boundary = domain_boundary, limits = limits, breaks = breaks, LEGEND = FALSE) +
        standard_plot_settings_fields()
      # same for each quantile (min,median,max)
      for (j in 1:3) {
        plot_list[[4 * (i - 1) + j + 1]] <- plot.field_points(qualitative_results$nodes, loadings[[name_model]][[indexes[[i]][j]]][, i], boundary = domain_boundary, size = 1.5) +
          standard_plot_settings_fields()
        plot_list_locs[[4 * (i - 1) + j + 1]] <- plot.field_points(qualitative_results$locations, loadings_locs[[name_model]][[indexes[[i]][j]]][, i], boundary = domain_boundary, size = 1.5) +
          standard_plot_settings_fields()
        plot_list_HR_clean[[4 * (i - 1) + j + 1]] <- plot.field_tile(qualitative_results$grid, loadings_HR[[name_model]][[indexes[[i]][j]]][, i], boundary = domain_boundary) +
          standard_plot_settings_fields()
        plot_list_HR[[4 * (i - 1) + j + 1]] <- plot.field_tile(qualitative_results$grid, loadings_HR[[name_model]][[indexes[[i]][j]]][, i], boundary = domain_boundary, limits = limits, breaks = breaks) +
          standard_plot_settings_fields()
      }
    }
