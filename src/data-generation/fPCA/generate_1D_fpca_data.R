# = ========================================================================== =
# - Script: generate_data_1D.R
# - Desc: Generates synthetic 1D FPCA-style data on a given domain and set of
#         locations using user-provided (or default) loading and mean generators.
# = ========================================================================== =


## Function: generate_1D_fpca_data
# - Args:
#   * domain: list containing at least $knots (1D grid of nodes)
#   * test_options: nested list with fields:
#       - $dimensions$n_stat_units
#       - $dimensions$n_comp
#       - $dimensions$n_locs
#       - $noise$NSR
#       - $noise$fPC1_specific_noise (optional logical)
#   * loadings_true_generator: function(locs, i) -> loading values for component i
#   * mean_generator: function(locs) -> mean field values
#   * seed: integer, RNG seed for reproducibility
# - Desc:
#   Builds spatial loadings on 1D knots and at sampled locations, samples scores,
#   constructs a mean function, adds noise based on the NSR, and returns true
#   and observed quantities for testing.
generate_1D_fpca_data <- function(domain, locations,
                                  test_options,
